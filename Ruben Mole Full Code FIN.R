...
test <- read_csv("class_data.csv", col_types = cols( #Reading in the data of all classes and there proportions
  class = col_character(),
  .default = col_number()
))
...
dat3 <- dat %>% # Extracting the second letter of the sample ID to label the location of each sample as either "North" or "South".
  mutate(
    ...
    map = case_when(
      substr(sampleid, 3, 3) == "C" ~ "South",
      substr(sampleid, 3, 3) == "S" ~ "South",
      substr(sampleid, 3, 3) == "N" ~ "North",
